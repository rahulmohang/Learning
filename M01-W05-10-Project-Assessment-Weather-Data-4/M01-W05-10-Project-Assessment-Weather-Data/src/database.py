...
class Database:
    ...
    # This method inserts the data in a new document. It assumes that any uniqueness check is done by the caller
    def update_single_data(self, collection,id,field,field_value):
        db_collection = self._db[collection]
        document = db_collection.find_one_and_update( 
            {"_id" : id},
                {"$set":
                    {field: field_value}
                },upsert=True
    ...
    # This method fetch the data from data based on query
