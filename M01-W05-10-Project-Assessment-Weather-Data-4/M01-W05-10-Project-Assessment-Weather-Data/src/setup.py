...
with open(RELATIVE_CONFIG_PATH+USER_COLLECTION+'.csv', 'r') as user_fh:
    for user_row in user_fh:
        ...
        if user_row:
            (username, email, role, device_exception) = user_row.split(',')
        user_data = {'username': username, 'email': email, 'role': role, 'device_exception': device_exception}
...
db_details = weather_dbh["weather_data"]
for doc in (db_details.aggregate([ {'$group' : {'_id' : {'device_id':"$device_id",'day':{"$dayOfMonth":"$timestamp"}}, 'total' : {'$sum' : "$value"} , 'Avgerage' : {'$avg' : "$value"}, 'min_value' : {'$min' : "$value"}, 'max_value' : {'$max' : "$value"}}},{ '$sort' : { 'device_id' : 1 ,'_id':1}}])):
    daily_report_collection=weather_dbh[DAILY_REPORT_COLLECTION]
...
for doc in (daily_report_collection.aggregate([{"$match": {"$expr":{ "$and": [{ "$gte": [ "$_id.day", 1 ]},{ "$lte": [ "$_id.day", 4 ] },{"$eq":["$_id.device_id",device_id]}]}}}])):
    print(doc)  